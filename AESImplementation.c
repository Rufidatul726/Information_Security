...
unsigned char Rcon[] = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b};
...
void expandKey(unsigned char *expandedKey, unsigned char *key, size_t expandedKeySize){
    int currentSize = 0;
    int rconIteration = 1;
    int i;
    unsigned char t[4] = {0}; 
    ...
    while (currentSize < expandedKeySize){
        for (i = 0; i < 4; i++)t[i] = expandedKey[(currentSize - 4) + i];
...
void mixColumns(unsigned char *state){
    ...
    for (i = 0; i < 4; i++){
        ...
        mixColumn(column);
        for (j = 0; j < 4; j++)state[(j * 4) + i] = column[j];
...
void createRoundKey(unsigned char *expandedKey, unsigned char *roundKey){
    int i, j;
    for (i = 0; i < 4; i++){
        for (j = 0; j < 4; j++)roundKey[(i + (j * 4))] = expandedKey[(i * 4) + j];
    }
}
...
char aes_encrypt(unsigned char *input, unsigned char *output,unsigned char *key){
    ...
    expandedKey = malloc(expandedKeySize * sizeof(char));

    for (i = 0; i < 4; i++){
        for (j = 0; j < 4; j++) block[(i + (j * 4))] = input[(i * 4) + j];
    }
...
void invMixColumn(unsigned char *column){
    ...
    column[0] = g(cpy[0], 14) ^ g(cpy[3], 9) ^ g(cpy[2], 13) ^ g(cpy[1], 11);
    column[1] = g(cpy[1], 14) ^ g(cpy[0], 9) ^ g(cpy[3], 13) ^ g(cpy[2], 11);
    column[2] = g(cpy[2], 14) ^ g(cpy[1], 9) ^ g(cpy[0], 13) ^ g(cpy[3], 11);
    column[3] = g(cpy[3], 14) ^ g(cpy[2], 9) ^ g(cpy[1], 13) ^ g(cpy[0], 11);
}
...
int main(int argc, char *argv[]){
    ...
    unsigned char plaintext[16] = {"Rufidatul Radium"};
    ...
    expandKey(expandedKey, key, 176);

    printf("\nPlaintext:\n");
    for (i = 0; i < 16; i++)printf("%X ", plaintext[i]);
    ...
    printf("\nDecryptedtext:\n");
    for (i = 0; i < 16; i++)printf("%X ", decryptedtext[i]);
